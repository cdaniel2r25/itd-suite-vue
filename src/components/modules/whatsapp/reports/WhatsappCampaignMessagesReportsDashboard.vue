<template>
  <v-container v-if="messagesStatusCount.total !== 0">
    ...
    fillCardList() {
      this.cardsList.forEach((card) => {
        if (card.label === 'Total procesados')
          ...
          card.value = this.messagesStatusCount.seen
        else if (card.label === 'Recibidos')
          card.value = this.messagesStatusCount.received
